...
\begin{abstract}
During pot loading for solid-state Baijiu distillation, localized steam
breakthroughs indicate where fermented grain should be deposited. We study
the resulting persistent-service control problem in a MuJoCo digital twin of
an ABB manipulator. An external thermal-localization module supplies clustered
target positions, and the robot moves an abstract deposition center over the
active locations. A Horizon-2
...
Pot loading (\emph{shangzeng}) is a critical operation in solid-state Baijiu
distillation~\cite{wang2024baijiu}. Operators observe steam breakthrough
locations and deposit fermented grain according to the ``cover upon
breakthrough'' practice while maintaining a light, uniform layer. Delayed or
inconsistent deposition can promote steam channeling, leakage, and nonuniform
loading. In the automated abstraction studied here, an external thermal
localization system supplies breakthrough coordinates and an ABB manipulator
moves a deposition center over the vessel. Targets arrive in spatially
clustered burst--lull cycles, remain active until served, and may accumulate
while the arm is in transit. The resulting persistent-service problem couples
geometric travel, waiting time, target age, and congestion.

This paper isolates the online routing and motion-command layer. It does not
evaluate thermal-image detection accuracy, granular-material dynamics, layer
uniformity, or final distillation quality. A simulated coverage event therefore
represents service of a detected breakthrough location rather than a complete
validation of the physical loading process.
...
\end{align}
For protocols with material-state sensing, the descriptor is
...
$\sigma_h=0.22$ m. Urgency augmentation is disabled in the evaluated protocol,
so it does not enter either $q_i$ or the H2 objective.
...
$(.40,.30,0,.10,.20)$.
Material-state observation is disabled in the evaluated protocol; hence
...
$(68,64,62,60)$, $(16,14,12,10)$, and $(8.0,7.5,6.5,5.8)$; the reported scale
...
cell, and the terminal model after a prespecified curriculum. The training
...
$921,600/3,200=288$ PPO updates. Model parameters are retained every 50
training episodes and at curriculum-stage boundaries, but all reported
learned results use the terminal model after episode 1,152. Held-out data do
not participate in model selection. Matched component variants use the same
prespecified hyperparameters, budget, and action interface; no common
hyperparameter search was completed. The historical Vanilla diagnostic was
not independently retuned under a full-budget protocol.
...
.888/.825; overlapping intervals make this robustness evidence. H3 provides
the requested planning-horizon sensitivity test. Because the learned policy
is conditioned on H2 actions, this comparison is not interpreted as evidence
of cross-planner residual transfer.
...
the learned controller is conditioned on H2. Finally, the $0.05$ s interval
...
persistent service; terminal-drain evaluation, cross-planner residual
transfer, and hardware timing remain future work.
