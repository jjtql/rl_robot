...
This work is motivated by robot-assisted pot loading in solid-state Baijiu
distillation. Localized steam breakthroughs are detected by thermal sensing,
...
$\sigma_h=0.22$ m. The reported protocol disables urgency augmentation, so it
does not enter either $q_i$ or the H2 objective.
...
$(.40,.30,0,.10,.20)$.
The reported protocol disables material-state observation; hence
...
$921,600/3,200=288$ PPO updates. Model states are retained every 50
training episodes and at curriculum-stage boundaries; the reported policy is
the terminal
model obtained after episode 1,152 under the prespecified selection rule.
Held-out data do not select models. Matched component variants use the
same fixed experimental settings, budget, and action interface; no common
hyperparameter search was completed. The historical Vanilla diagnostic was
not independently retuned under a full-budget protocol. The archived
episode-level records contain reward, coverage, SLA, residual scale, recurrent
counters, prediction events, and elapsed steps. These measurements verify
curriculum and schedule execution; held-out metrics do not select models.
